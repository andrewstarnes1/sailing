...
namespace KWS
{
    internal class KW_PyramidBlur
    {
        ...
        {
            if (_tempSeparableRT == null)  _tempSeparableRT = KWS_CoreUtils.RTHandleAllocVR(width, height, colorFormat: target.rt.graphicsFormat); 
            if (_blurMaterial    == null) _blurMaterial = CreateMaterial(BlurShaderName);

            cmd.SetGlobalFloat(_sampleScale, blurRadius);

            var pass = blurRadius < 1.5f ? 2 : 4;
            cmd.BlitTriangleRTHandle(source,           Vector4.one, _tempSeparableRT, _blurMaterial, ClearFlag.None, Color.clear, pass);
            cmd.BlitTriangleRTHandle(_tempSeparableRT, Vector4.one, target,           _blurMaterial, ClearFlag.None, Color.clear, pass + 1);
        }
